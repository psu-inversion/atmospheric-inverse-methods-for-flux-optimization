...
For the identical twin setup, we use the same covariance matrix in the
noise generation and the inversion. For a single assimilation window,
there are analytical distributions for the :math:`\chi^2` statistic
(unsurprisingly, :math:`\chi^2(M)` where :math:`M = N_{obs times}
N_{sites}` is the number of observations, for the innovations
(:math:`N(0, HBH^T + R)` as assumed), and for the innovation sample
covariance matrix (inverse Wishart is defined to be this
distribution).

A K-S test confirms this for sample size 200 for the :math:`\chi^2`
statistic: I know of no equivalents for multivariate or matrix
distributions.
...
SP_ERROR_CORRELATION = SP_ERROR_CORRELATION_FUN.make_matrix(NY, NX)
TM_ERROR_CORRELATION = TM_ERROR_CORRELATION_FUN.make_matrix(N_FLUX_TIMES)
ERROR_CORRELATION = scipy.linalg.kron(TM_ERROR_CORRELATION,
                                      SP_ERROR_CORRELATION)

# Do this part before or after the kronecker product?
STDS = np.ones(N_FLUX_TIMES * N_GRID_POINTS)
DIAG_STDS = np.diag(STDS)
ERROR_COVARIANCE = DIAG_STDS.dot(ERROR_CORRELATION.dot(DIAG_STDS))
print(ERROR_COVARIANCE.shape)
OBS_STDS = np.ones((N_OBS_TIMES, N_SITES))
OBSERVATION_COVARIANCE = np.diag(np.square(OBS_STDS.reshape(-1)))

ERR_COV_CUBE = iris.cube.Cube(
    ERROR_COVARIANCE.reshape(N_FLUX_TIMES, NY, NX,
                             N_FLUX_TIMES, NY, NX),
...
if __name__ == "__main__":
    ...
    for i in range(N_RUNS):
        ...
        prior_cov = ERROR_COVARIANCE[:N_TIMES_BACK * N_GRID_POINTS,
